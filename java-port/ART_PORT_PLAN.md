...
>   - ✅ Game-layer resolver: `.mes` loader (`MesFile`/`Mes`, ported from `mes.c`) + `NameResolver` (ports `name_resolve_path` for critter / interface / scenery / container / monster / unique_npc / eye_candy). Registered via `TigArt.setFilePathResolver`; a dwarf critter now renders **purely from a `tig_art_id_t`** (id → `art\critter\DFM\DFMBNSAd.art` → decode → screen). Tested (`MesFileTest`, `NameResolverTest`).
>   - Remaining: the `a_name_*`-driven types (tile/wall/portal/item/light/roof/facade) and `name_normalize_aid`'s missing-art fallback; then `TigArt.blit(TigArtBlitInfo)` is a thin shim over `draw(artId,…)` once the struct type is generated.
